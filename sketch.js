...
function preload()
{
  ...
  endImg = loadAnimation("Monkey_01.png");
  }
...
function setup() {
  ...
  //background
   back=createSprite(0,600,displayWidth,displayHeight);
...
//gameState is play
    ...
    textSize(20);
    fill("white"); 
    text("Survival Time: "+score,400,150);   
}

function food(){        //function for creating food at every 130 sec
 if(frameCount%300==0){
   banana = createSprite(displayWidth-10,displayHeight-10,40,10);
...
if(frameCount%400==0){
  obs = createSprite(displayWidth-10,displayHeight-30 ,10,40);
...
function over(){ 
  ...
  foodGroup.destroyEach();         //shows game over on screen
  textSize(20);
  fill("white"); 
